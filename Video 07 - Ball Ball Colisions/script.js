...
function mainLoop(timestamp) {
    ...
    BALLZ.forEach((b, index) => {
        b.drawBall();
        if (b.player){
            keyControl(b, input);// INDEX TO BE ADDED LATER
        }

        for(let i = index + 1; i < BALLZ.length; i++)
            if(collisionDetectionBB(BALLZ[index], BALLZ[i])){
                penetrationResponseBB(BALLZ[index], BALLZ[i]);
    }
        b.display();
    });
    ...
    let Ball4 = new Ball(390, 200, 30);
    let Ball5 = new Ball(210, 300, 40); 
    let Ball6 = new Ball(140, 380, 35);
    let Ball7 = new Ball(500, 120, 30);
    let Ball8 = new Ball(400, 400, 40); 
    let Ball9 = new Ball(360, 340, 35);
    Ball1.player = true;
...
$(document).ready(function()
{
    ...
    $('#code_ver').text('html ver 1527 * * * JS ver 1527');
